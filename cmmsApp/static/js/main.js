...
/* ===== CONFIG ===== */
const SOLAR = {
  revealSelector: '.reveal-lv-mv-hv-swt',
  inViewClass: 'in-view-lv-mv-hv-swt',
...
(function initLogosPager() {
  const wrap = document.querySelector('.logos-wrap-lv-mv-hv-swt');
  const track = document.getElementById('logos-track-lv-mv-hv-swt');
  const dotsWrap = document.getElementById('dots-lv-mv-hv-swt');
  if (!wrap || !track || !dotsWrap) return;

  const dots = Array.from(dotsWrap.querySelectorAll('.dot-lv-mv-hv-swt'));
  const RESUME_DELAY = 3500; // ms after click before continuous scroll resumes
  let resumeTimer = null;

  // set active dot helper
  function setActiveDot(idx) {
    dots.forEach((d, i) => d.classList.toggle('is-active-lv-mv-hv-swt', i === idx));
  }
  setActiveDot(0);
  ...
  function enterManualMode() {
    track.classList.add('manual-lv-mv-hv-swt');
  ...
  function resumeContinuous() {
    ...
    track.classList.remove('manual-lv-mv-hv-swt');
  ...
  const ro = new ResizeObserver(() => {
    const active = dots.findIndex(el => el.classList.contains('is-active-lv-mv-hv-swt'));
    if (active > -1 && track.classList.contains('manual-lv-mv-hv-swt')) {
  ...
  wrap.addEventListener('mouseenter', () => {
    if (!track.classList.contains('manual-lv-mv-hv-swt')) {
      track.style.animationPlayState = 'paused';
    }
  });
...
(function initImpactCounters() {
  const items = document.querySelectorAll('.stat-value-lv-mv-hv-swt-impact');
...
(function initSolutionsToggle() {
  const grid = document.getElementById('solutions-grid-lv-mv-hv-swt-solution');
  const btn = document.getElementById('solutions-toggle-btn-lv-mv-hv-swt-solution');
  if (!grid || !btn) return;

  function setState(expanded) {
    grid.classList.toggle('is-collapsed-lv-mv-hv-swt-solution', !expanded);
...
(function initProjectsCarousel() {
  const viewport = document.getElementById('projects-viewport-lv-mv-hv-swt-projects');
  const prevBtn = document.querySelector('.prev-lv-mv-hv-swt-projects');
  const nextBtn = document.querySelector('.next-lv-mv-hv-swt-projects');
...
(function initSolarTypes() {
  const tabs = Array.from(document.querySelectorAll('.tab-btn-lv-mv-hv-swt-types'));
  const panels = {
    'on-grid': document.getElementById('panel-on-grid-lv-mv-hv-swt-types'),
    'off-grid': document.getElementById('panel-off-grid-lv-mv-hv-swt-types'),
    'hybrid': document.getElementById('panel-hybrid-lv-mv-hv-swt-types')
  };
  ...
  function activate(type) {
    ...
    tabs.forEach(btn => {
      ...
      btn.classList.toggle('is-active-lv-mv-hv-swt-types', isActive);
    ...
    Object.entries(panels).forEach(([key, el]) => {
      const show = key === type;
      if (!el) return;
      el.classList.toggle('is-active-lv-mv-hv-swt-types', show);
      el.hidden = !show;
      if (show) {
  ...
  document.querySelector('.tabs-lv-mv-hv-swt-types')?.addEventListener('keydown', (e) => {
    const idx = tabs.findIndex(b => b.classList.contains('is-active-lv-mv-hv-swt-types'));
...
/* ==========================================================
   Scoped tabs for all .section-types-lv-mv-hv-swt-types
   (no global getElementById; supports multiple instances)
   ========================================================== */
(function initAllSolarTypeTabs() {
  document.querySelectorAll('.section-types-lv-mv-hv-swt-types').forEach(section => {
    const tabsWrap = section.querySelector('.tabs-lv-mv-hv-swt-types');
    if (!tabsWrap) return;

    const tabs = Array.from(section.querySelectorAll('.tab-btn-lv-mv-hv-swt-types'));
    const panels = Array.from(section.querySelectorAll('.panel-lv-mv-hv-swt-types'));
    ...
    function activate(btn) {
      ...
      tabs.forEach(t => {
        ...
        t.classList.toggle('is-active-lv-mv-hv-swt-types', isActive);
      ...
      panels.forEach(p => {
        ...
        p.hidden = !show;
        p.classList.toggle('is-active-lv-mv-hv-swt-types', show);
    ...
    tabsWrap.addEventListener('keydown', (e) => {
      ...
      e.preventDefault();
      const current = tabs.findIndex(t => t.classList.contains('is-active-lv-mv-hv-swt-types'));
      const dir = e.key === 'ArrowRight' ? 1 : -1;
    ...
    activate(tabs.find(t => t.classList.contains('is-active-lv-mv-hv-swt-types')) || tabs[0]);
  });
})();
...
(() => {
  const targets = document.querySelectorAll(
    '.reveal-top-lv-mv-hv-swt1-why-us, .reveal-up-lv-mv-hv-swt1-why-us, .reveal-right-lv-mv-hv-swt1-why-us, .reveal-left-lv-mv-hv-swt1-why-us'
  );

  const io = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-lv-mv-hv-swt1-why-us');
...
(() => {
  const targets = document.querySelectorAll(
    '.reveal-top-lv-mv-hv-swt-csc-products, .reveal-left-lv-mv-hv-swt-csc-products, .reveal-right-lv-mv-hv-swt-csc-products, .reveal-up-lv-mv-hv-swt-csc-products'
  );
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting){
        e.target.classList.add('reveal-in-lv-mv-hv-swt-csc-products');
        io.unobserve(e.target);
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  ...
  const close = modal.querySelector('.modal-close-csc-lv-mv-hv-swt-csc-products');
  ...
  document.querySelectorAll('.view-doc-csc-lv-mv-hv-swt-csc-products').forEach(a => {
  ...
  document.querySelectorAll('.request-download-csc-lv-mv-hv-swt-csc-products').forEach(btn => {
    btn.addEventListener('click', () => {
  ...
  modal.querySelector('.modal-backdrop-csc-lv-mv-hv-swt-csc-products')
